...
def scrape_data(link):
    ...
    if rowCount == 625:
      try:
        df1 = pd.DataFrame(table_data, columns=table_headers)
        excel_filename = 'web_scraped_page_1_12_test.xlsx'
        df1.to_excel(excel_filename, index=False)
      except (e):
        print("errorrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr",e)
      else:pass
df['Scraped Data'] = df['Links'].apply(scrape_data)  # Replace 'URL Column' with your column name

df = pd.DataFrame(table_data, columns=table_headers)
excel_filename = 'web_scraped_page_25_50.xlsx'
